...
def runLaser(lasDexarm, fileName):    
...
def initializeArduino(wait=True): #This function declares the hard-coded variable for the Arduino. 
    global laserArduinoSerial
    laserArduinoSerial = serial.Serial("COM22", 115200, timeout=0.2)
    if wait==True:
        time.sleep(3)
    ...
    while True:
        ...
        if(len(messageIncoming)>0):
            ...
            if(chr(messageIncoming[1])=='D'):
                print("Laser door closed..") #Added a line of code to output when this function finishes. 
                break;
...
def LaserDoorOpen():
    ...
    while True:
        ...
        if(len(messageIncoming)>0):
            print(messageIncoming)
            if(chr(messageIncoming[1])=='D'):
                print("Laser door opened.") #Added a line of code to output when this function finishes. 
                break;

def DoorOpen():
    for i in range(100):
        laserArduinoSerial.write(chr(int(100)).encode('ascii'))
        time.sleep(0.5)
        laserArduinoSerial.flush()
        time.sleep(0.5)
        laserArduinoSerial
