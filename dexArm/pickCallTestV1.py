...
# Constants
SPEED_ROBOT = 6000  # Original 6000
...
def pick_fuzzy_foot(dexarm, index_x, index_y):
	...
	dexarm.go_home() #At the start of each pick, go home. This one is good to have and unobstrusive. 
	...
	#dexarm.go_home() We dont't actually need this, I just forgot to change from PICK_Z to the new HOVER_Z with the updated version of the code. 
...
# Constants for triangle computation
PLACEMENT_RADIUS = 70 / math.sqrt(3)  # Radius for equilateral triangle placement
TRIANGLE_ANGLE = math.radians(120)  # 120 degrees for equilateral triangle

def calculate_triangle_positions():
    # Calculate positions of the triangle points around the center
    positions = []
    for i in range(3):
        angle = TRIANGLE_ANGLE * i - math.pi/2 # Starting angle tweaked since it was 90 degrees rotated before. . 
...
def placeFuzzyFeet(dexarm, pick_indices, num_to_place):
    ...
    for i in range(num_to_place):
        ...
        dexarm.move_to(placement_x, placement_y, HOVER_Z - PLACEMENT_OFFSET_Z, mode="G1", feedrate=SPEED2_ROBOT*2)
        # Move back up, slowly
        dexarm.move_to(placement_x, placement_y, HOVER_Z + PLACEMENT_OFFSET_Z, mode="G1", feedrate=SPEED2_ROBOT*2) #Go up a bit extra, since it's still moving too fast at the end. 
        # Move the rest of the way up to Z=0
        dexarm.move_to(placement_x, placement_y, 0, mode="G1", feedrate=SPEED_ROBOT)
        print(f"Placed fuzzy foot {i+1} at [{placement_x}, {placement_y}]")
        

    #When finished, move backwards so you're out of the way. 
    #dexarm.move_to(0, 50, 0, mode="G1", feedrate=SPEED_ROBOT)

# Example usage
pick_indices = [(1, 1), (2, 1), (3, 1)]
placeFuzzyFeet(dexarm2, pick_indices, 1)
